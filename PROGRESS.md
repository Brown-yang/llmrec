...
当前竞赛最高分仍是 **v1.0.0 = 0.8596**。⚠️ **澄清"champion到底做没做处理"**：v1.0.0 **没做任何数据增强/加外部数据/改配比**（纯官方种子SFT数据），但**做了两个格式处理**：①**drop-long@4096**（丢弃超长样本，32480→30705，丢的~1775条几乎全是懂用户长历史）②**剥CoT + 强制 non-thinking**。其中 non-thinking 是高分功臣，drop-long 无意中饿了懂用户（→exp5用cutoff8192修复）。所以"什么都没改"在"没加数据"上对、在"没做格式处理"上**不准确**。之前的「改进」都没超过它：exp1=0.8529、exp4=0.8063、raw=0.7575、v1.0.2=0.7354、v1.0.3=0.7212——因为都在动 champion 配方的**正确部分**（增强/thinking/解冻itemic/智能截断），越动越差。

**转折（2026-07-07）：从评测 log 逆向分析发现 champion 有两个"权重-数据错配"缺口**——**懂用户**(25%权重却只3.6%数据，长样本被4096截断丢了61%)、**懂世界**(12.5%权重却0官方数据，全靠基座)。**→ 最终结论（2026-07-08）：SFT 已彻底到顶，champion 0.8596 是天花板。** 系统试过 exp5(+懂世界)=0.8446、exp8(接champion续训+修好的增强+懂世界)=0.8437，**都低于champion**。铁律：**加数据 = 拿懂推荐换懂世界、还净亏**（懂推荐必掉~0.03，懂世界最多赚~0.02）。连"接champion续训 + 评测核对过的干净增强"都救不了。根因（FAQ baseline印证）：懂物料冻结、懂世界权重小且稀释懂推荐、懂推荐/懂用户卡在泛化问题（评测95%新物品，SFT只会记忆）。**详见 §7.1 规律6/7。突破 0.9 只剩 RL 一条路。**
...
|---|---|---|---|---|---|---|---|
| **v1.0.0** | 官方32k, 1ep, **纯LoRA, non-thinking, drop-long@4096(→30705)**, 无增强 | **0.8596** | 0.1533 | 0.1006 | **0.4726** | 0.1331 | **CHAMPION**。做了格式处理(drop-long+剥CoT/non-thinking),但无数据增强 |
...
| **exp5** | cutoff 8192(恢复懂用户)+ 懂世界5000(新), 1ep, LoRA, non-thinking | **0.8446** | 0.1533 | 0.0959 | 0.4408 | **0.1546** | **懂世界+0.0215成功✅**但懂推荐稀释-0.032❌+懂用户恢复失败-0.005❌(评测截断长输入,FAQ Q5印证)。net-0.015 |
| ~~exp6~~ | 从base从头训 champion配方+懂世界 | **弃** | — | — | — | — | 训到24%时用户澄清要"接champion续训"而非从头,停掉改做exp7 |
| **exp7** | **接champion-1881续训** + (官方+懂世界5000), LR5e-5, cutoff4096 | **未提交** | — | — | — | — | eval_loss单调降1.185→1.175(零过拟合),div_sa16.97健康。未花评测测(按规律估~0.85) |
| **exp8** | **接champion-1881续训** + (官方+修好的增强懂推荐20k/懂物料5k+懂世界5k=60705), LR5e-5 | **0.8437** | 0.1533 | 0.1016 | 0.4390 | 0.1498 | **增强+续训仍失败**：懂推荐-0.034(增强30k带偏)、懂世界+0.017(被增强挤配比,比exp5少)。net-0.016 |
...
6. **⭐ "加数据 = 拿懂推荐换懂世界、还净亏"（2026-07-08，exp5/exp8 三次证实，最终钉死SFT）**：不管怎么加数据（从头/续训、增强/懂世界、干净/脏），**懂推荐必掉~0.03，懂世界最多赚~0.02，净亏~0.015**。
   - champion 懂推荐0.4726/懂世界0.1331/总0.8596
   - exp5(+懂世界,从头) 懂推荐0.4408(-0.032)/懂世界0.1546(+0.021)/总0.8446
   - exp8(+修好的增强+懂世界,**接champion续训**) 懂推荐0.4390(-0.034)/懂世界0.1498(+0.017)/总0.8437
   - **连"接champion续训 + 评测核对过合格的干净增强"都救不了**——懂推荐一加数据就被带偏。**SFT 到此为止，别再加数据实验。**
7. **SFT 动不了的根因（FAQ baseline印证，见§7.4）**：懂物料**冻结**(0.1533,SFT改不动)；懂世界能+0.02但权重小、且加它必稀释懂推荐；懂推荐(50%)/懂用户(25%)卡住因为**评测95%是新物品(泛化)，SFT只会记忆**。→ **唯一出路是 RL(GRPO) 撬动懂推荐**。
...
### 7.4 官方 FAQ 澄清（2026-07-07，来自官方 FAQ.txt）

**① Baseline(预训练,未微调)分数 = 0.6655，拆解揭示"SFT到底改变了什么"：**

| 维度 | baseline(预训练) | champion(SFT) | SFT作用 |
|---|---|---|---|
| 懂物料 | 0.1533 | 0.1533 | **完全不变** |
| 懂用户 | 0.0055 | 0.1006 | +0.095 |
| 懂推荐 | 0.3680 | 0.4726 | +0.105 |
| 懂世界 | 0.1387 | 0.1331 | **-0.006(SFT略伤!)** |

- **懂物料被冻在0.1533**：官方证实 SFT 改不动它（预训练就是这值）。→ **别再想SFT提升懂物料**（exp1的0.184是异常，exp4的0.1226是被坏数据拖低）。
- **懂世界：SFT反而略伤**（灾难性遗忘）→ **exp5加懂世界数据至少能把0.1331拉回0.1387+**（这是官方给的、exp5会有增益的铁证）。
- **懂用户/懂推荐才是SFT的杠杆**——正是 exp5 补的方向。

**② 评测子项顺序（官方Q3，从左到右）**：总分, 懂物料, 懂用户(选择), 懂用户(主题), 懂推荐(视频), 懂推荐(电商), 懂推荐(广告), 懂推荐(直播), 世界知识。
- 用此映射 exp4：懂推荐-视频0.0768(**最低**,但数据最多60%) / 电商0.1292 / 广告0.1442 / 直播0.1098；懂用户-主题(topic_gen)0.0349(**全场最低,最难**)。
- 子项分数是**乘过权重后**的（官方按难度归一化），总分=8子项之和；4场景权重一致。

**③ 外部数据明确允许**（官方Q3）：可引入外部通识/自建推荐数据；但**复现阶段要提供构造脚本+数据**（我们的 build_shijie.py/build_*_augment.py 都在git，合规）。种子SFT数据"仅作参考，可自行清洗配比"。

**④ ⚠️ 序列长度警告**（官方Q5）："序列长度已选最佳，不需调整"（针对平台精调）。**exp5自训练把cutoff改到8192**：自训练能改，但若**评测端把懂用户长输入截断到固定长度，8192的长上下文训练可能白费**。→ exp5若懂用户没涨，可能是评测截断，不一定是方法错。

**⑤ 其他**：RL代码不提供需自实现（Q3）；LoRA提交=adapter_model.safetensors+adapter_config.json（Q1）；线下建议transformers v5.3.0（我们用5.6.0）。
