...
**转折（2026-07-07）：从评测 log 逆向分析发现 champion 有两个"权重-数据错配"缺口**——**懂用户**(25%权重却只3.6%数据，长样本被4096截断丢了61%)、**懂世界**(12.5%权重却0官方数据，全靠基座)。→ **exp5**(cutoff8192恢复懂用户 + 懂世界5000)= **0.8446**，拆解出3个独立结论：**①懂世界补齐成功 +0.0215✅**(0.1331→0.1546，超baseline 0.1387，CMMLU/GSM8K/LogiQA有效)；②cutoff8192恢复懂用户**失败-0.005**(评测端截断长输入，FAQ Q5印证)；③懂推荐被稀释**-0.032**(占比62.5%→51%)。→ **exp6**（champion + **仅加懂世界**，退回cutoff4096，不恢复懂用户）训练中：只留有效的懂世界、去掉无效的cutoff8192，懂推荐稀释更小(53.8%)，**最有希望超champion**。
...
| **exp6** | champion配方(cutoff4096,drop-long)+**仅加懂世界5000**,不恢复懂用户, 1ep, LoRA, non-thinking | **训练中** | — | — | — | — | 拆exp5只留有效部分：懂推荐占比51%→53.8%稀释更小。**最有希望超champion** |
